...
| **Audio guidance** | Cross-platform TTS via [`flutter_tts`](https://pub.dev/packages/flutter_tts); long text split on sentence boundaries for cancel |

| **Wireframe avatar** | Pseudo-3D stick-figure animations during active workouts (3 exercises animated so far) |

| **Active workout UI** | Live playback state, exercise wireframe, cancel controls |
...
├── docs/                # Design docs (workout playback, wireframes)
...
- [**Workout playback**](docs/workout-playback.md) — Player, announcer, gate, sentence splitting, cancel/pause design

- [**Wireframe avatars**](docs/wireframe-avatars.md) — Adding exercise animations, projection debug
...
flutter test lib/workout/test/                  # workout engine + announcer
...
flutter run lib/workout/test/announcer_test.dart   # announcer smoke test (log backend)

flutter run lib/workout/test/player_test.dart      # full workout playback

flutter run lib/workout/test/tts_smoke_runner.dart   # TTS smoke test (Windows/web)
...
- **Workout player** — Pause/resume UI (gate infrastructure in place)
...
Ideas welcome — use this README, `docs/`, and `TODO.md` as starting points for brainstorming.
